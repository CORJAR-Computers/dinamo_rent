...
class TestDirectAccessMethods:
    ...
    def test_get_logging_config(self, ini_file_full):
        ...
        assert log_cfg["max_size_mb"] == 10
        assert log_cfg["backup_count"] == 7
        assert log_cfg["audit_enabled"] is True
        assert log_cfg["audit_retention_days"] == 90
        assert "logs_app" in log_cfg["directory"]
...
class TestFallbackValues:
    ...
    def test_logging_config_fallbacks(self, ini_file_minimal):
        ...
        cfg = AppConfig(config_path=ini_file_minimal)
        log_cfg = cfg.get_logging_config()
        assert log_cfg["max_size_mb"] == 5  # fallback
        assert log_cfg["level"] == "INFO"  # fallback
        assert log_cfg["audit_enabled"] is True  # fallback
