...
class TestFlujoCompletoRenta:
    ...
    def test_flujo_con_retraso_y_cargos_extra(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Late", "apellidos": "Return",
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Nissan", "modelo": "Versa",
            "tipo": "Automóvil", "transmision": "Mecánica",
            "combustible": "Gasolina", "kilometraje": 20000.0,
            "estado": "Disponible",
        })

        hoy = date.today()
        renta_id = RentaService.crear({
            "placa": placa, "id_cliente": cliente_id,
            "nombre_cliente": "Late Return",
            "fecha_recogida": hoy, "hora_recogida": time(10, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=2),
            "hora_retorno": time(10, 0),
            "dias_calculados": 2, "valor_dia": Decimal("80000"),
    ...
    def test_flujo_pago_total_al_cierre(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Full", "apellidos": "Payment",
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Chevrolet", "modelo": "Spark",
            "kilometraje": 5000.0, "estado": "Disponible",
        })
        ...
        renta_id = RentaService.crear({
            ...
            "fecha_recogida": hoy, "hora_recogida": time(9, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=1),
            "hora_retorno": time(9, 0),
            "dias_calculados": 1, "valor_dia": Decimal("120000"),
        ...
        PagoService.registrar({
            "id_renta": renta_id, "monto": "50000",
            "metodo_pago": "Efectivo", "concepto": "Abono",
        })

        # Register final payment at closure
        PagoService.registrar({
            "id_renta": renta_id, "monto": "70000",
            "metodo_pago": "Tarjeta", "concepto": "Pago final",
        })
    ...
    def test_flujo_devuelve_mismo_dia(self):
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Renault", "modelo": "Logan",
            "kilometraje": 30000.0, "estado": "Disponible",
        })
        ...
        renta_id = RentaService.crear({
            ...
            "fecha_recogida": hoy, "hora_recogida": time(8, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=5),
            "hora_retorno": time(8, 0),
            "dias_calculados": 5, "valor_dia": Decimal("90000"),
...
class TestFlujoErrores:
    ...
    def test_crear_renta_vehiculo_no_disponible(self):
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Test", "modelo": "X",
            "kilometraje": 100.0, "estado": "Disponible",
        })
    ...
    def test_crear_renta_cliente_lista_negra(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Black", "apellidos": "Listed",
    ...
    def test_cerrar_renta_ya_cerrada(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Double", "apellidos": "Close",
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Test", "modelo": "Y",
            "kilometraje": 200.0, "estado": "Disponible",
        })
    ...
    def test_crear_renta_sin_cliente(self):
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Test", "modelo": "Z",
            "kilometraje": 300.0, "estado": "Disponible",
        })
    ...
    def test_pago_monto_cero(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Zero", "apellidos": "Pay",
            "nombre_completo": "Zero Pay",
            "estado": "Activo",
        })
        cliente_id = ClienteService.buscar(doc)[0]["id"]
        AutoService.guardar({
            "placa": placa, "marca": "Test", "modelo": "W",
            "kilometraje": 400.0, "estado": "Disponible",
        })
...
class TestFlujoMultiplesRentas:
    ...
    def test_auto_rentado_varias_veces(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Sequential", "apellidos": "Renter",
        ...
        AutoService.guardar({
            "placa": placa, "marca": "Kia", "modelo": "Picanto",
            "kilometraje": 10000.0, "estado": "Disponible",
        })
        ...
        r1_id = RentaService.crear({
            ...
            "fecha_retorno": hoy + datetime.timedelta(days=1),
            "hora_retorno": time(10, 0),
            "total": Decimal("80000"), "abono": Decimal("80000"),
        })
        assert AutoService.obtener(placa)["estado"] == "Rentado"

        # Return 1
        RentaService.cerrar(r1_id, {
            "fecha_devolucion_real": hoy + datetime.timedelta(days=1),
            "hora_devolucion_real": time(10, 0),
            "km_final": "10200",
        })
        ...
        r2_id = RentaService.crear({
            ...
            "hora_retorno": time(9, 0),
            "total": Decimal("160000"), "abono": Decimal("100000"),
        })
        assert AutoService.obtener(placa)["estado"] == "Rentado"

        # Return 2
        RentaService.cerrar(r2_id, {
        ...
        r3_id = RentaService.crear({
            ...
            "total": Decimal("160000"), "abono": Decimal("160000"),
        })
        assert AutoService.obtener(placa)["estado"] == "Rentado"

        RentaService.cerrar(r3_id, {
    ...
    def test_rentas_activas_after_operations(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc_a,
            "nombres": "Active", "apellidos": "One",
            "nombre_completo": "Active One",
            "estado": "Activo",
        })
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc_b,
            "nombres": "Active", "apellidos": "Two",
        ...
        r1 = RentaService.crear({
            "placa": p1, "id_cliente": cid_a,
            "nombre_cliente": "Active One",
            "fecha_recogida": hoy, "hora_recogida": time(10, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=3),
            "hora_retorno": time(10, 0),
            "total": Decimal("150000"), "abono": Decimal("50000"),
        })
        r2 = RentaService.crear({
            "placa": p2, "id_cliente": cid_b,
        ...
        r3 = RentaService.crear({
            "placa": p3, "id_cliente": cid_a,
            "nombre_cliente": "Active One",
            "fecha_recogida": hoy, "hora_recogida": time(12, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=1),
            "hora_retorno": time(12, 0),
            "total": Decimal("50000"),
        })
...
class TestFlujoCambioVehiculo:
    ...
    def test_cambiar_vehiculo_en_renta_activa(self):
        ...
        ClienteService.guardar({
            "tipo_doc": "Cédula", "no_doc": doc,
            "nombres": "Swap", "apellidos": "Vehicle",
        ...
        for p in [placa_a, placa_b]:
            AutoService.guardar({
                "placa": p, "marca": "Test", "modelo": "Swap",
                "kilometraje": 5000.0, "estado": "Disponible",
            })

        hoy = date.today()
        renta_id = RentaService.crear({
            "placa": placa_a, "id_cliente": cliente_id,
            "nombre_cliente": "Swap Vehicle",
            "fecha_recogida": hoy, "hora_recogida": time(10, 0),
            "fecha_retorno": hoy + datetime.timedelta(days=2),
            "hora_retorno": time(10, 0),
            "total": Decimal("200000"), "abono": Decimal("50000"),
        })
