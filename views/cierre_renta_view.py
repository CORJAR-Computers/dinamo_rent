...
BUG FIX: metodos inconsistentes corregidos. Estilos via QSS class-based.
"""
...
class CierreRentaDialog(BaseDialog):
    ...
    def _setup_ui(self):
        ...
        root.addWidget(
            build_dialog_header(
                "📋",
                "Procesar Devolucion" + (f" — Renta #{self.id_renta}" if self.id_renta else ""),
                "Cierre de renta y liquidacion final",
            )
        )
        ...
        self.lbl_info.setProperty("class", "title")
        ...
        gb_dev.setLayout(f_dev)
        body_lay.addWidget(gb_dev)

        gb_cos = QGroupBox("Costos Adicionales")
        f_cos = QFormLayout()
        self.sp_dias_extra = QDoubleSpinBox()
        self.sp_dias_extra.setReadOnly(True)
        self.sp_dias_extra.setSuffix(" dias")
        self.sp_mora = QDoubleSpinBox()
        self.sp_mora.setRange(0, 1e7)
        self.sp_mora.setPrefix("$ ")
        self.sp_mora.setEnabled(False)
        self.sp_otros = QDoubleSpinBox()
        self.sp_otros.setRange(0, 1e7)
        self.sp_otros.setPrefix("$ ")
        self.sp_otros.valueChanged.connect(self._recalcular)
        self.txt_obs = QLineEdit()
        self.txt_obs.setPlaceholderText("Danos, gasolina, etc.")
        f_cos.addRow("Tiempo Extra:", self.sp_dias_extra)
        f_cos.addRow("Costo Tiempo Extra:", self.sp_mora)
        f_cos.addRow("Otros (Danos/Gasolina):", self.sp_otros)
        f_cos.addRow("Observacion:", self.txt_obs)
        gb_cos.setLayout(f_cos)
        body_lay.addWidget(gb_cos)

        frame_tot = QFrame()
        frame_tot.setProperty("class", "summary-card")
        lay_tot = QVBoxLayout(frame_tot)
        self.lbl_pactado = QLabel("Pactado: $0")
        self.lbl_pactado.setProperty("class", "section")
        self.lbl_total = QLabel("TOTAL A PAGAR: $0")
        self.lbl_total.setProperty("class", "title")
    ...
    def _cargar_datos(self):
        ...
        try:
            ...
            ModernMessageBox.warning(self, "Error", e.mensaje_usuario)
            self.reject()
    ...
    def _guardar(self):
        ...
        try:
            ...
            ToastNotification(
                self.window(),
                "Renta finalizada correctamente. El vehículo está Disponible.",
                "success",
            )
            self.accept()
        except DinamoBaseError as e:
