...
class InformeRepositorySA:
    ...
    def obtener_ingresos_por_vehiculo() -> Dict[str, float]:
        ...
        with get_session() as session:
            resultados = (
                session.query(
                    Renta.placa, func.coalesce(func.sum(Renta.total), 0).label("total_ingresos")
                )
                .filter(
                    Renta.estado == "Finalizado",
                    Renta.placa.isnot(None),
                )
                .group_by(Renta.placa)
                .all()
            )
        ...
        with get_session() as session:
            resultados = (
                session.query(
                    ...
                    func.coalesce(func.sum(MantenimientoVehiculo.total_mantenimiento), 0).label(
                        "total_manto"
                    ),
                )
                .filter(
                ...
                .group_by(MantenimientoVehiculo.placa)
                .all()
            )
    ...
    def obtener_gastos_por_vehiculo() -> Dict[str, float]:
        ...
        with get_session() as session:
            resultados = (
                session.query(
                    Gasto.placa, func.coalesce(func.sum(Gasto.monto), 0).label("total_gastos")
                )
                .filter(
                    Gasto.placa.isnot(None),
                )
                .group_by(Gasto.placa)
                .all()
            )
