...
"""
views/comparendos_view.py — Gestion de Multas y Comparendos. Estilos via QSS class-based.
"""
    ...
    def _setup_ui(self):
        ...
        root.addWidget(
            build_dialog_header(
                ...
                "Foto-multas y comparendos — vinculación automática al cliente",
            )
        )
        ...
        self.sp_monto.setRange(0, 100_000_000)
        self.sp_monto.setPrefix("$ ")
        self.txt_obs = QTextEdit()
        self.txt_obs.setMaximumHeight(80)
        self.txt_obs.setPlaceholderText("Lugar de la infraccion, codigo, etc...")
...
class ComparendosWidget(BaseWidget):
    ...
    def __init__(self, session_id: str = None):
        ...
        banner = create_banner(
            "🚔",
            "Control de Comparendos y Fotomultas",
            "Gestion de multas y asignacion a clientes",
            self.cargar_datos,
        )
        ...
        try:
            for i, c in enumerate(ComparendoService.listar()):
                ...
                if not c.get("cliente_nombre"):
                    it_cli.setForeground(QBrush(QColor("#888888")))
    ...
    def _mostrar_menu(self, pos):
        ...
        if acc == ac_pagar:
            self._cambiar_estado(id_comp, "Pagado")
        elif acc == ac_apelar:
            self._cambiar_estado(id_comp, "Apelado")

    def _cambiar_estado(self, id_comp: int, estado: str):
        try:
            ComparendoService.cambiar_estado(id_comp, estado)
            self.mostrar_exito(f"Estado actualizado a {estado}")
            self.cargar_datos()
        except DinamoBaseError as e:
